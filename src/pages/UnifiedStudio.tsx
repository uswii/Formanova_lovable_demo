...
export default function UnifiedStudio() {
  ...
  const [formanovaCategoryView, setFormanovaCategoryView] = useState<'ecom' | 'editorial'>('ecom');
                ...
                <Tabs defaultValue="formanova" className="w-full">
                  ...
                  <TabsContent value="formanova">
                    <div className="flex gap-0">
                      {/* Left: category list — secondary nav */}
                      <div className="flex flex-col flex-shrink-0 w-[72px] pr-2 pt-0.5 border-r border-border/15">
                        {([
                          ...
                          <button
                            ...
                            className={`text-left w-full py-2 pl-2 pr-1 font-mono text-[9px] uppercase tracking-[0.1em] leading-snug transition-colors border-l-2 ${
                              formanovaCategoryView === cat.key
                                ? 'border-foreground text-foreground'
                                : 'border-transparent text-muted-foreground/50 hover:text-muted-foreground hover:border-border/40'
                            }`}
                          >
                            <span className="block">{cat.label}</span>
                            <span className={`block mt-0.5 text-[8px] tracking-wider ${formanovaCategoryView === cat.key ? 'text-muted-foreground' : 'text-muted-foreground/30'}`}>
                              {cat.count} models
                            </span>
                          </button>
                        ))}
                      ...
                      {/* Right: model grid */}
                      <div className="flex-1 min-w-0 pl-3 max-h-[480px] overflow-y-auto pr-1">
