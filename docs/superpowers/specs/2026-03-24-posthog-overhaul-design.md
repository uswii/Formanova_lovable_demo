...
posthog.capture('category_selected', {
  category: category.id,         // 'necklace' | 'earrings' | 'rings' | 'bracelets' | 'watches'
...
**CAD:** `TextToCAD.tsx` → generate handler, when `balance < cost` (uses `performCreditPreflight` directly — different pattern from photo studio, same intent).
```ts
posthog.capture('paywall_hit', {
  category: jewelryType,   // 'cad' for TextToCAD
  steps_completed: 2,      // photo: upload(1) + model(2); CAD: prompt(1)
})
```
...
```ts
{
  category: jewelryType,   // only in UnifiedStudio context
}
```
...
- [ ] Click Generate with zero credits → `paywall_hit` fires with `steps_completed: 2`
- [ ] Click Generate in CAD with zero credits → `paywall_hit` fires with `category: 'cad'`
